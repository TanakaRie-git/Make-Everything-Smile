...
**作例/図:**
- 顔での笑顔方向の効き(サニティ): [v3_faces_sanity.jpg](assets/vae/v3_faces_sanity.jpg)
...
- **失敗A: 物体が人間の顔に化ける** — グローバル潜在(v2)。
  作例: [v2_backpack_collapse.jpg](assets/vae/v2_backpack_collapse.jpg)
- **失敗B: 性別のもつれ**(笑顔強化で女性化)。→ **男女別方向**でキャンセル。
- → **空間潜在**で物体の構図・色を保持(v3)。強度で「物体のまま→顔がうっすら→顔が支配的」と遷移。
  作例: [v3_icecream.jpg](assets/vae/v3_icecream.jpg)
...
- スクラッチ VAE-GAN を Diffusion と同じ FacesInThings crop で編集すると、
  ...
  作例(推奨: assets へ書き出し): `outputs/diffusion_compare/baseline/_overview.png`
...
- **事前学習 SD-VAE を土台**にし、同じ潜在方向演算を回すと:
  ...
  - 物体を保ったまま笑顔が宿る。
  作例(推奨: assets へ): `outputs/pretrained_ae/compare/_overview.png`
...
- 残課題「暖色ドリフト」(一律ベクトル加算の副作用)を **色保持(`--keep-color`)** で除去。
  ...
  作例(発表キービジュアル候補・推奨 assets へ):
  `outputs/pretrained_ae/compare_fit_kc/_overview.png` /
  `outputs/pretrained_ae/compare_fit_both/000015457_compare.png`(ワッフルの明確な笑み)

### 参考: スクラッチ版の別解 — ピクセル空間ブレンド(単一・大写し物体)
- スクラッチ VAE-GAN でも `--blend`(`元画像 + 目口マスク × (decode(z+d)−decode(z))`)で
  物体のシャープな画素を保ち笑顔差分だけ重ねると、単一果物で笑顔が宿る。
  作例: [blend_gas_pump.jpg](assets/vae/blend_gas_pump.jpg) /
  [fruit_lemons.jpg](assets/vae/fruit_lemons.jpg)
...
| 使いどころ | 画像 | 見せたいこと |
|---|---|---|
| 手法概要(方向が効く証拠) | [v3_faces_sanity.jpg](assets/vae/v3_faces_sanity.jpg) | 顔で笑顔方向が機能 |
| 失敗A(物体崩壊) | [v2_backpack_collapse.jpg](assets/vae/v2_backpack_collapse.jpg) | グローバル潜在の限界 |
| 対策(強度遷移) | [v3_icecream.jpg](assets/vae/v3_icecream.jpg) | α で連続制御 |
| 幕2: 人間化(スクラッチ) | `outputs/diffusion_compare/baseline/_overview.png` ※assetsへ | 同一入力で人間に化ける |
| 幕3: 事前学習土台で鮮明化 | `outputs/pretrained_ae/compare/_overview.png` ※assetsへ | 弱点は土台の問題 |
| 幕4: 物体自身が笑う(色保持) | `outputs/pretrained_ae/compare_fit_kc/_overview.png` ※assetsへ | Diffusion と同じ土俵 |
| 幕4: キービジュアル | `outputs/pretrained_ae/compare_fit_both/000015457_compare.png` ※assetsへ | ワッフルの明確な笑み |
| 参考: ブレンド/果物 | [blend_gas_pump.jpg](assets/vae/blend_gas_pump.jpg) / [fruit_lemons.jpg](assets/vae/fruit_lemons.jpg) | スクラッチ版の別解 |

> ※印は今回の新規実験の出力(`vae/outputs/` 配下・gitignore)。発表前に代表フレームを
> `docs/assets/vae/` へ書き出して貼る。
